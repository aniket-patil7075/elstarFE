...
import {
  apiSaveSignatureImage,
  getEstimateById,
} from "@/views/Dealer/Services/WorkflowService";
...
const AuthorizeEmail = () => {
  const [selectedServicesTotal, setSelectedServicesTotal] = useState<number>(0);
    ...
    if (estimate.services.length > 0) {
      ...
      estimate.services.map((service: any) => {
        services.push({
          ...
          isAuthorized: true,
          serviceGrandTotal: service.serviceGrandTotal,
        });
      });
  ...
  useEffect(() => {
    ...
    const total = allSelectedServices.reduce((acc: any, service: any) => {
      return service.isAuthorized
        ? acc + (service.serviceGrandTotal || 0)
        : acc;
    }, 0);
  ...
  const Step1 = () => {
    return (
      <div className="text-center">
        ...
        />
        <h3 className="mb-2">
          <ul>You have aleady authorized below service</ul>
        </h3>
      ...
      <div className="min-h-screen bg-gray-100 p-2 sm:p-4 pb-24">
        ...
        <div className="flex justify-between items-center bg-white shadow p-3 sm:p-4 mb-4 rounded-lg">
          <div>
            ...
            {/* <h3  className="text-base sm:text-lg font-semibold">Order Id : {orderId}</h3> */}
            <p className="text-xs sm:text-sm text-gray-600">
              Order #{estimate.orderNo}
            </p>
          </div>
              ...
              <div className="space-y-4">
                ...
                <div className="flex flex-col">
                  {estimate &&
                    ...
                    estimate.services.map((service: any, idx: number) => (
                            ...
                            {service.labors.length > 0 && (
                              <div className="mt-4 overflow-x-auto">
                                ...
                                <Table className="min-w-full text-xs sm:text-sm">
                                  <THead>
                                    <Tr>
                                      <Th className="text-left text-xs sm:text-sm">
                                        Labor
                                      </Th>
                                      <Th className="text-left text-xs sm:text-sm">
                                        Technician
                            ...
                            {service.subcontract.length > 0 && (
                              <div className="mt-6 overflow-x-auto">
                                ...
                                <Table className="min-w-full text-xs sm:text-sm">
                                  ...
                                  <TBody>
                                    {service.subcontract.map(
                                      (subcontract: any, idx: number) => (
                                        <Tr key={idx}>
                                          <Td>
                                            {subcontract.subcontractName || "-"}
                                          </Td>
                                          <Td>{subcontract.vendor || "-"}</Td>
                                          <Td>{subcontract.cost || "$0.00"}</Td>
                                          <Td>
                                            {subcontract.price || "$0.00"}
                                          </Td>
                                          <Td>
        ...
        <div
          className="border-t border-gray-300 bg-white rounded-lg shadow fixed bottom-0 w-[96%] md:w-[73%] p-3 flex flex-col space-y-2 mx-auto mb-3"
          ...
          <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-2 sm:p-4">
            <div className="bg-white rounded-lg shadow-lg w-[90%] md:w-[30%] max-w-lg mx-auto">
              ...
              <div className="bg-slate-50 p-3 sm:p-4 rounded-md text-base sm:text-lg font-bold text-center m-4 sm:m-6">
                Total Amount: ${selectedServicesTotal}
              </div>
              ...
              <div className="p-4 sm:p-6 mt-4">
                ...
                >
                  Authorize ${selectedServicesTotal}
                </button>
              </div>
            </div>
          </div>
        ...
        {showSignaturePopup && (
          <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
            <div className="bg-white p-6 rounded-lg shadow-lg w-[90%] md:w-[40%] flex flex-col space-y-4">
              {/* Signature Heading */}
              <h3 className="text-lg font-bold text-center">Add Signature</h3>
              ...
              <div className="flex justify-end space-x-4">
                <button
                  ...
                  onClick={() => setShowSignaturePopup(false)} // Close signature popup
                >
                  Cancel
                </button>
                <button
